...
![GlanceVeil wordmark](browser-extension/assets/glanceveil-wordmark.svg)

GlanceVeil is a shoulder-surfing filter for webpages, Obsidian notes, and tmux
panes: readable to you, slower to parse at a glance. It
...
GlanceVeil has no developer-operated service or telemetry. Its local data handling
is described in the [privacy policy](PRIVACY.md).

![GlanceVeil displaying an Obsidian note through Greek-looking glyphs](docs/glanceveil-obsidian-preview.svg)
...
On a fresh install, GlanceVeil automatically opens a focused one-minute local setup.
...
- Open the extension popup to select a script, strength, and renderer. Choose
  **Gregg Simplified · experimental** to try connected outlines. Changing an
  appearance control also turns GlanceVeil on for the current tab.
...
ordinary DOM text nodes. GlanceVeil intercepts only the text-drawing calls for the
...
Load or reload the document after installing or updating GlanceVeil. At startup,
...
Actions taken from the GlanceVeil popup reload a Google Docs document automatically.
...
3. Copy `main.js`, `manifest.json`, and `styles.css` from `dist/obsidian` into
   that directory.
4. Reload Obsidian and enable **GlanceVeil** under **Community plugins**.
...
On mobile, sync the complete `.obsidian/plugins/glanceveil` directory with the
vault, restart Obsidian, and enable GlanceVeil under **Community plugins**. The
ribbon provides the mobile toggle because Obsidian does not expose its desktop
status bar on mobile.

## Install in tmux

GlanceVeil for tmux opens the current pane through the shared Unicode transformer
...
close the GlanceVeil view and return. Ordinary text, Enter, Escape, arrows, function
keys, control shortcuts, and application mouse events remain available to the
source application.

With [Tmux Plugin Manager](https://github.com/tmux-plugins/tpm), add GlanceVeil to
...
immediately. Press prefix followed by `Ctrl-k` to revisit settings; GlanceVeil does
...
tmux has no DOM-like overlay layer. GlanceVeil therefore captures the source pane's
...
GlanceVeil is designed to slow down quick visual reading by another person.
...
locally to the selected source pane; GlanceVeil never records them.
...
```bash
xcrun safari-web-extension-converter "/full/path/to/glanceveil/browser-extension"
```

Open the generated project, run the macOS app target, then enable GlanceVeil in
...
`font-builder/build-glanceveil-font.py` creates a personal display font from a
local font that already contains Greek glyphs:

```bash
fontforge -script font-builder/build-glanceveil-font.py \
  "/path/to/source-font.ttf" \
  "$HOME/Desktop/GlanceVeil-Regular.ttf"
```
...
If GlanceVeil is useful to you, you can optionally
[support its maintenance on Ko-fi](https://ko-fi.com/mickadlr). Support does
not unlock features or grant commercial-use rights. See [SUPPORT.md](SUPPORT.md)
for details.

## License

GlanceVeil is proprietary source-available software, not open source. Unmodified

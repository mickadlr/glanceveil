...
Kalima browser extension. It never writes transformed text to Markdown
files.
...
Kalima uses the Obsidian ribbon for its one-tap toggle on desktop and mobile.
...
`.obsidian/plugins/kalima` directory with the vault, including `main.js`,
`manifest.json`, and `styles.css`. Then restart Obsidian mobile and enable
Kalima under **Settings → Community plugins**. The plugin has no Node.js,
